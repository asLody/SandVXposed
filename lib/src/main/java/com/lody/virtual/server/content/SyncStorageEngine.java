...
public class SyncStorageEngine extends Handler {
    ...
    public static class AuthorityInfo {
        ...
        AuthorityInfo(AuthorityInfo toCopy) {
            ...
            for (PeriodicSync sync : toCopy.periodicSyncs) {
                ...
                periodicSyncs.add(mirror.android.content.PeriodicSync.clone(sync));
            }
        }
        ...
        AuthorityInfo(Account account, int userId, String authority, int ident) {
            ...
            PeriodicSync sync = new PeriodicSync(account, authority,
                    new Bundle(),
                    DEFAULT_POLL_FREQUENCY_SECONDS);
            long flexTime = calculateDefaultFlexTime(DEFAULT_POLL_FREQUENCY_SECONDS);
        ...
        AuthorityInfo(ComponentName cname, int userId, int ident) {
            ...
            final PeriodicSync periodicSync =
                    new PeriodicSync(account, authority,
                            new Bundle(),
                            DEFAULT_POLL_FREQUENCY_SECONDS);
            mirror.android.content.PeriodicSync.flexTime.set(periodicSync, calculateDefaultFlexTime(DEFAULT_POLL_FREQUENCY_SECONDS));
    ...
    private final SparseArray<ArrayList<VSyncInfo>> mCurrentSyncs
            = new SparseArray<ArrayList<VSyncInfo>>();
     ...
     * The flex time is capped at a percentage of the DEFAULT_POLL_FREQUENCY_SECONDS.
     */
            ...
            try {
                ...
                if (add) {
                    ...
                    for (int i = 0, N = authority.periodicSyncs.size(); i < N; i++) {
                        ...
                        if (mirror.android.content.PeriodicSync.syncExtrasEquals(
                                toUpdate.extras,
                                syncInfo.extras)) {
                            if (toUpdate.period == syncInfo.period &&
                                    mirror.android.content.PeriodicSync.flexTime.get(toUpdate) == mirror.android.content.PeriodicSync.flexTime.get(syncInfo))
                            {
                                // Absolutely the same.
                                return;
                            }
    ...
    public boolean isSyncActive(Account account, int userId, String authority) {
        synchronized (mAuthorities) {
            for (VSyncInfo syncInfo : getCurrentSyncs(userId)) {
...
//     */
    public VSyncInfo addActiveSync(SyncManager.ActiveSyncContext activeSyncContext) {
        ...
        synchronized (mAuthorities) {
            ...
            syncInfo = new VSyncInfo(authority.ident,
    ...
    public long insertStartSyncEvent(Account accountName, int userId, int reason,
        ...
        reportChange(ContentResolverCompat.SYNC_OBSERVER_TYPE_STATUS);
        return id;
    }
    ...
    public List<VSyncInfo> getCurrentSyncsCopy(int userId) {
        synchronized (mAuthorities) {
            final List<VSyncInfo> syncs = getCurrentSyncsLocked(userId);
            final List<VSyncInfo> syncsCopy = new ArrayList<VSyncInfo>();
    ...
    private List<VSyncInfo> getCurrentSyncsLocked(int userId) {
        ArrayList<VSyncInfo> syncs = mCurrentSyncs.get(userId);
        if (syncs == null) {
            syncs = new ArrayList<VSyncInfo>();
        ...
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                for (PeriodicSync periodicSync : authority.periodicSyncs) {
                    ...
                    out.attribute(null, "flex", Long.toString(flexTime));
