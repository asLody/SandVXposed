...
public class NativeLibraryHelperCompat {
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    private static boolean isVM32(Set<String> supportedABIs) {
        if (supportedABIs == null || supportedABIs.isEmpty()) {
            return true;
        }

        for (String supportedAbi : supportedABIs) {
            if ("armeabi".endsWith(supportedAbi) || "x86".equals(supportedAbi) || "mips".equals(supportedAbi) || "armeabi-v7a".equals(supportedAbi)) {
                return true;
            }
        }
    ...
    public static Boolean isApk32(String szApkPath) {
        return isVM32(getABIsFromApk(szApkPath));
    }
}
